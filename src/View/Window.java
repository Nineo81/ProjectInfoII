...
public class Window implements SizeableObserver, Resumer, Runnable {
    ...
    public Window(int type) { //type 1= game - type 2 = menu
        ...
        if (type==1) {
            ...
            window.setBounds(200, 0, 1600, 1020);
            ...
            JFrame itemFrame = new JFrame("Item");
            itemFrame.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
            itemFrame.setBounds(700, 400, 500, 300);
            itemFrame.getContentPane().setBackground(Color.gray);
            this.window = itemFrame;
    ...
    public void run() {
        try {
            Thread.sleep(200);
            this.window.getContentPane().add(this.itemPanel);
    ...
    public void setSkillTree(){
        this.skillTreePanel.attachModifier(((Player)(((Game) observers.get(0)).getGameObjects()).get(0)));
    }




    public void attachItemObserver(Player player, GameObject item){
        System.out.println(item.getClass().getName());
        this.openedItem=item;
        itemPanel = new ItemPanel(this, openedItem);
        this.itemPanel.setObserver(player);
    }
    ...
    @Override
    public void attachResumer(ResumerObserver ro){
        observers.add(ro);
